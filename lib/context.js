 ...
 *
 * @param {String|Number} tagOrLine The tag or line to find
 * @param from
 * @returns {*}
 */
Context.prototype.find = function (tagOrLine, from) {
  ...
  if (_.isString(tagOrLine)) {
    for (i = from; i < length; i++) {
      if (_.startsWith(tokens[i].text, tagOrLine)) {
        return tokens[i];
      }
    }
  } else if (_.isNumber(tagOrLine)) {
    for (i = from; i < length; i++) {
      if (tagOrLine === 0) {
        return tokens[i];
      }
  ...
  throw new Error('Can not find ' + (_.isNumber(tagOrLine) ? 'line ' : 'tag ') + tagOrLine + ' from index ' + from);
};
